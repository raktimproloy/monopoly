...
export default function ChatBox({ logs }: ChatBoxProps) {
  ...
  return (
    <div className="w-full h-full p-4 glass-card flex flex-col gap-3.5 select-none relative">
      ...
      {/* Terminal logs list */}
      <div className="flex-1 overflow-y-auto pr-1 glass-item rounded-xl p-3 flex flex-col gap-2 font-mono text-[10px] leading-relaxed text-slate-300">
      ...
      <form onSubmit={handleSubmit} className="flex gap-2">
        ...
        <button
          type="submit"
          disabled
          className="p-2.5 glass-item text-slate-500 rounded-lg flex items-center justify-center cursor-not-allowed opacity-50 transition-colors"
        >
          <Send size={14} />
        </button>
      </form>
