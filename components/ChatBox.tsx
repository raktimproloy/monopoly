...
export default function ChatBox({ entries, userId }: ChatBoxProps) {
  ...
  return (
    <div className={`w-full p-4 glass-card flex flex-col gap-3.5 select-none relative transition-all duration-300 ${isExpanded ? 'h-[400px] shrink-0' : 'h-14 shrink-0'}`}>
      <div 
        className="flex items-center justify-between cursor-pointer group"
        onClick={() => setIsExpanded(!isExpanded)}
      >
        <h3 className="text-base font-orbitron font-extrabold tracking-widest text-white uppercase flex items-center gap-2 group-hover:text-cyber-blue transition-colors">
          <Terminal size={16} className="text-[#8B5CF6]" />
          TACTICAL TELEMETRY
        </h3>
        <button className="text-slate-400 group-hover:text-cyber-blue transition-colors">
        ...
        <>
          ...
          <form onSubmit={(e) => e.preventDefault()} className="flex gap-2">
            <input
              ...
              className="flex-1 px-3 py-2 text-[11px] font-mono glass-input text-white placeholder-white/50 focus:outline-none cursor-not-allowed uppercase shrink-0"
            />
            <button
              type="button"
              disabled
              className="p-2.5 glass-item text-slate-500 rounded-lg flex items-center justify-center cursor-not-allowed opacity-50 shrink-0"
            >
              <Send size={14} />
            </button>
          </form>
